...
# * Create a basebackup
# * Create a table, each second insert a current timestamp (in a background)
# * Run pgbench, fill with 1M of rows
# * Generate some data (+-512Mi)
# * Kill insert-script job
# * Run pg_dumpall, save result as plain SQL
# * Drop all postgres processes, drop PGDATA directory
# * Restore PGDATA from basebackup, add recovery.signal, add restore_command
# * Rename all *.partial wal files in wal archives
# * Start a cluster
# * Run pg_dumpall after cluster is ready
# * Run diff between pg_dumpall results (before/after)
# * Check insert-script logs, check that table contains last inserted row
# * Compare wal dirs  (two directories should match 100%, filenames/content)
# * Cleanup wal dirs, run wal-archivers with a new timeline
# * Compare wal dirs again
