...
- In `backup` mode, it performs a **full base backup** of your PostgreSQL cluster on a configured schedule -  
  for instance, **once every three days** - using **streaming basebackup**, 
  with optional **compression** and **encryption**. 
  The built-in cron scheduler enables fully automated backups without requiring external orchestration.
